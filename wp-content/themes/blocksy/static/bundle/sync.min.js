 ...
 * Google as part of the polymer project is also subject to an additional IP
 * rights grant found at http://polymer.github.io/PATENTS.txt
 */Object.defineProperty(F,"__esModule",{value:!0});const P=d,j=v;class T extends j.NodeVisitor{stringify(e){return this.visit(e)||""}[P.nodeType.stylesheet](e){let t="";for(let o=0;o<e.rules.length;++o)t+=this.visit(e.rules[o]);return t}[P.nodeType.atRule](e){return`@${e.name}`+(e.parameters?` ${e.parameters}`:"")+(e.rulelist?`${this.visit(e.rulelist)}`:";")}[P.nodeType.rulelist](e){let t="{";for(let o=0;o<e.rules.length;++o)t+=this.visit(e.rules[o]);return t+"}"}[P.nodeType.comment](e){return`${e.value}`}[P.nodeType.ruleset](e){return`${e.selector}${this.visit(e.rulelist)}`}[P.nodeType.declaration](e){return null!=e.value?`${e.name}:${this.visit(e.value)};`:`${e.name};`}[P.nodeType.expression](e){return`${e.text}`}[P.nodeType.discarded](e){return""}}F.Stringifier=T,
/**
 * @license
 * Copyright (c) 2016 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
